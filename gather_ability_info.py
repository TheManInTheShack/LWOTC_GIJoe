...
def gather_ability_data(idata,tdata):
    ...
    for abil in abilities:
        ...
        # ----------------------------------------------------------------------
        rec = (abil,len(abilities[abil]['usage']), abilities[abil]['lowrank'],abilities[abil]['slot'],friendlyname,longdesc,helptext,flyover,promo)
        abil_recs.append(rec)

    # --------------------------------------------------------------------------
    # Make that into a data frame
    # --------------------------------------------------------------------------
    adata = pd.DataFrame(abil_recs, columns=['ability','instances','lowrank','slot','friendlyname','longdesc','helptext','flyover','promo'])
