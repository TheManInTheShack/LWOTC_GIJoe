class MZ_Effect_VoidPrison_BloodDrain extends X2Effect;

var int DamagePerAction;
var float HealthReturnMod, PsiToDamageMod;

var bool bRed;	//if the effect should use blood magic red drain instead of templar blue.

simulated protected function OnEffectAdded(const out EffectAppliedData ApplyEffectParameters, XComGameState_BaseObject kNewTargetState, XComGameState NewGameState, XComGameState_Effect NewEffectState)
{
	local XComGameState_Unit TargetUnit, SourceUnit;
	local int StartingHP, DamagedHP, DifferenceHP, i, DamagePower;
	...
	if (ActionsToTick > 0)
	{
		...
		DamagePower = DamagePerAction;
		If ( PsiToDamageMod != 0 )
		{
			DamagePower += Round(PsiToDamageMod*SourceUnit.GetCurrentStat(eStat_PsiOffense));
		}

		for (i = 0; i < ActionsToTick; ++i)
		{
			StartingHP = TargetUnit.GetCurrentStat(eStat_HP);
			TargetUnit.TakeEffectDamage(self, DamagePower, 0, 0, ApplyEffectParameters, NewGameState);
...
simulated function AddX2ActionsForVisualization_Tick(XComGameState VisualizeGameState, out VisualizationActionMetadata ActionMetadata, const int TickIndex, XComGameState_Effect EffectState)
{
	...
	if (ActionMetadata.StateObject_NewState.IsA('XComGameState_Unit'))
	{
		...
		if ( HealthReturnMod > 0 )
		{
			...
			if ( bRed )
			{
				EffectAction.EffectName = "ANIM_MZPsi_BloodMagic.ParticleSystem.P_Void_Conduit_Drain_Tether";
			}
			else
			{
			...
			if ( bRed )
			{
				EffectAction.EffectName = "ANIM_MZPsi_BloodMagic.ParticleSystem.P_Void_Conduit_Drain";
			}
			else
			{
		...
		if ( HealthReturnMod > 0 )
		{
			...
			if ( bRed )
			{
				EffectAction.EffectName = "ANIM_MZPsi_BloodMagic.ParticleSystem.P_Void_Conduit_Drain_Templar";
			}
			else
			{
