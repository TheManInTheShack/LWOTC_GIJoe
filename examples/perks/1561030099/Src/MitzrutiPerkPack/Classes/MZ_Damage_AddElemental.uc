...
function WeaponDamageValue GetBonusEffectDamageValue(XComGameState_Ability AbilityState, XComGameState_Unit SourceUnit, XComGameState_Item SourceWeapon, StateObjectReference TargetRef)
{
	...
	If ( AbilityState.IsMeleeAbility() && !bNoDoubleGrip && !(bIgnoreBaseDamage && BonusDamageScalar < 1.0f) )
	{
		//allow the bonuses from double grip to be scaled by this damage effect.
		bDoubleGrip = IsDoubleGrip(SourceUnit, AbilityState);
	}

	If ( ( Element == 'melee' || (AbilityState.IsMeleeAbility() && bIgnoreBaseDamage == false) ) && TargetUnit.HasSoldierAbility('VulnerabilityMelee', true))
	{
		...
		if ( bDoubleGrip )
		{
			...
			{
				//here we're trying to prevent doubling up on the double grip bonus. for full conversion abilities that have >=1.0f mult.
				Round((WeaponTemplate.BaseDamage.Damage + GetBreakthroughBonus(WeaponTemplate, History) + GetDoubleGripDamage(WeaponTemplate) )* BonusDamageScalar) + AddDamage - GetDoubleGripDamage(WeaponTemplate);
			}
			else
			{
