...
var array<name> RandomItemNames;
...
simulated protected function OnEffectAdded(const out EffectAppliedData ApplyEffectParameters, XComGameState_BaseObject kNewTargetState, XComGameState NewGameState, XComGameState_Effect NewEffectState)
{
	...
	If ( RandomItemNames.Length > 1 )
	{
		//this is the roulette case: select an item from the list at random to add.
		ItemTemplate = ItemTemplateMgr.FindItemTemplate(RandomItemNames[`SYNC_RAND(RandomItemNames.length)]);
	}
	else
	{
		//this is the normal case: a single item added.
