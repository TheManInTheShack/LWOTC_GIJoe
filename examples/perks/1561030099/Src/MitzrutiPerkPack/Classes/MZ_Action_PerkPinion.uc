class MZ_Action_PerkPinion extends X2Action_BlazingPinionsStage2; // config(IriPsiOverhaul);
//based on the iridar's soulstorm one.
...
Begin:
	...
	for( TimeDelayIndexB = 0; TimeDelayIndexB < AbilityContext.InputContext.MultiTargets.Length; ++TimeDelayIndexB )
	{
		Sleep(`SYNC_FRAND() * 0.3f * GetDelayModifier());
		AddProjectiles(TimeDelayIndexB);
	}
