class MZ_Aim_PsiAttack extends X2AbilityToHitCalc_StandardAim config(MZPerkWeapons);

var config int Base;
var config float PsiFactor, ModPsiFactor, PsiCritFactor, ModPsiCritFactor;
var config array<int> PsiAim_Range;
	...
	{				
		if (!bIndirectFire)
		{
			...
			{	
				...
				OffenseValue = Base + ( UnitState.GetBaseStat(eStat_PsiOffense) * PsiFactor );
				AddModifier(OffenseValue, class'XLocalizedData'.default.PsiOffenseLabel, m_ShotBreakdown, eHit_Success, bDebugLog);
				//modifiers from effects that boost psi
				UnitState.GetStatModifiers(eStat_PsiOffense, StatMods, StatModValues);
				for (i = 0; i < StatMods.Length; ++i)
				{
					AddModifier(int(StatModValues[i] * ModPsiFactor), StatMods[i].GetX2Effect().FriendlyName, m_ShotBreakdown, eHit_Success, bDebugLog);
				}
		...
		if (bAllowCrit)
		{
			...
			//modifiers from psi score
			AddModifier(( UnitState.GetBaseStat(eStat_PsiOffense) * PsiCritFactor ), class'XLocalizedData'.default.PsiOffenseLabel, m_ShotBreakdown, eHit_Crit, bDebugLog);
			UnitState.GetStatModifiers(eStat_PsiOffense, StatMods, StatModValues);
			for (i = 0; i < StatMods.Length; ++i)
			{
				AddModifier(int(StatModValues[i] * ModPsiCritFactor), StatMods[i].GetX2Effect().FriendlyName, m_ShotBreakdown, eHit_Crit, bDebugLog);
			}
...
function int GetWeaponRangeModifier(XComGameState_Unit Shooter, XComGameState_Unit Target, XComGameState_Item Weapon)
{
	...
	if (Shooter != none && Target != none && Weapon != none)
	{
		...
		if (WeaponTemplate != none)
		{
			...
			if (PsiAim_Range.Length > 0)
			{
				if (Tiles < PsiAim_Range.Length)
					Modifier = PsiAim_Range[Tiles];
				else  //  if this tile is not configured, use the last configured tile					
					Modifier = PsiAim_Range[WeaponTemplate.RangeAccuracy.Length-1];
			}
		}
	}
