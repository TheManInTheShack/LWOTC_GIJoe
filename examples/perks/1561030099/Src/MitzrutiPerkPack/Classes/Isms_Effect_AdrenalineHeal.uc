class Isms_Effect_AdrenalineHeal extends X2Effect config(MZPerkWeapons);

var float PercentHealthPerMana;
...
simulated protected function OnEffectAdded(const out EffectAppliedData ApplyEffectParameters, XComGameState_BaseObject kNewTargetState, XComGameState NewGameState, XComGameState_Effect NewEffectState)
{
	...
	if (Ability != none && TargetUnit != none)
	{
		...
		{
			HealAmount = Round(PercentHealthPerMana * ManaEffectState.FocusLevel * (TargetUnit.GetMaxStat(eStat_HP) - TargetUnit.GetCurrentStat(eStat_HP)) / 100);
		}
