class MZ_Effect_AutoMendShield extends X2Effect_Persistent;
...
function bool RegenerationTicked(X2Effect_Persistent PersistentEffect, const out EffectAppliedData ApplyEffectParameters, XComGameState_Effect kNewEffectState, XComGameState NewGameState, bool FirstApplication)
{
	...
	if ( OldTargetState.GetCurrentStat(eStat_ShieldHP) >= OldTargetState.GetMaxStat(eStat_ShieldHP)) {return false;}
...
defaultproperties
{
	EffectName="MZBarrierCharger"
	EffectTickedFn=RegenerationTicked
	HealthRegeneratedName="MZBarrierChargerShieldRestored"
	MaxHealAmount=-1
}