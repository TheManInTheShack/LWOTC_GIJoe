...
class MZ_Effect_LifeSteal extends X2Effect config(MZPerkWeapons);
...
simulated protected function OnEffectAdded(const out EffectAppliedData ApplyEffectParameters, XComGameState_BaseObject kNewTargetState, XComGameState NewGameState, XComGameState_Effect NewEffectState)
{
	...
	if( (Ability != none) && (TargetUnit != none) )
	{
		...
		if( (SourceUnit != none) && (OldTargetUnit != none) )
		{
			...
			LifeAmountMultiplier = ((LowHealthVamp * (SourceUnit.GetMaxStat(eStat_HP) - SourceUnit.GetCurrentStat(eStat_HP)) / SourceUnit.GetMaxStat(eStat_HP)) + FlatVamp)/100;
		...
		LifeAmount =   int(float(LifeAmount) * ( float(default.PersistHealPercent) / 100 ));

		SourceUnit.LowestHP += LifeAmount;

		// sanity check, to prevent unit's hit points from growing over time
		if ( SourceUnit.LowestHP > SourceUnit.HighestHP ) {
			SourceUnit.LowestHP = SourceUnit.HighestHP;
		}
	}
}
