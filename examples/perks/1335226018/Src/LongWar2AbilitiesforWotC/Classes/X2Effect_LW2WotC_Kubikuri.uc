...
class X2Effect_LW2WotC_Kubikuri extends X2Effect_Persistent config (LW_SoldierSkills);
...
function int GetAttackingDamageModifier(XComGameState_Effect EffectState, XComGameState_Unit Attacker, Damageable TargetDamageable, XComGameState_Ability AbilityState, const out EffectAppliedData AppliedData, const int CurrentDamage, optional XComGameState NewGameState)
{
    ...
    if (AbilityState.GetMyTemplateName() == 'LW2WotC_Kubikuri')
    {
        if(AppliedData.AbilityResultContext.HitResult == eHit_Crit)
        {
            ...
            if(SourceWeapon != none) 
            {
                ...
                if(TargetUnit != none)
                {
                    ...
                    //`LOG ("Kubikuri Target" @ TargetUnit.GetMyTemplateName() @ "CurrentHP:" @ string(TargetUnit.GetCurrentStat(eStat_HP)) @ "MAXHP:" @ string (TargetUnit.GetMaxStat(eStat_HP)) @ "Current HP must be less than " @ string (TargetUnit.GetMaxStat(eStat_HP) * class'X2Ability_PerkPackAbilitySet'.default.KUBIKURI_MAX_HP_PCT/100));
                    if (TargetUnit.GetCurrentStat(eStat_HP) <= TargetUnit.GetMaxStat(eStat_HP) * class'X2Ability_LW2WotC_ActivatedAbilitySet'.default.KUBIKURI_MAX_HP_PCT / 100) 
                    {               
                        //`LOG ("Kubikuri dealing" @ int(2 * (TargetUnit.GetCurrentStat(eStat_HP)+TargetUnit.GetCurrentStat(eStat_ShieldHP)+TargetUnit.GetCurrentStat(eStat_ArmorMitigation)+CurrentDamage)) @ "damage.");
