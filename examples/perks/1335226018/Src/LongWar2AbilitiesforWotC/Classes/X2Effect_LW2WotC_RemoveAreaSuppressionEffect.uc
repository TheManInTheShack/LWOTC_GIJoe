    ...
    foreach History.IterateByClassType(class'XComGameState_Effect', EffectState)
    {
        ...
        if (EffectState.ApplyEffectParameters.SourceStateObjectRef.ObjectID == ApplyEffectParameters.SourceStateObjectRef.ObjectID) 
        {
            ...
            if (ShouldRemoveEffect(EffectState, PersistentEffect)) // basic check that the effectname matches
            {
                ...
                if (MatchesTarget || class'X2Effect_LW2WotC_AreaSuppression'.static.ShouldRemoveAreaSuppression(SourceState, NewGameState, true)) // remove if either matches target or area suppression is shutting down
                {
...
{
    local XComGameState_Effect EffectState;
    local X2Effect_LW2WotC_AreaSuppression Effect;
    ...
    foreach VisualizeGameState.IterateByClassType(class'XComGameState_Effect', EffectState)
    {
        if (EffectState.bRemoved)
        {
            ...
            {
                Effect = X2Effect_LW2WotC_AreaSuppression(EffectState.GetX2Effect());
                if (Effect != none)
