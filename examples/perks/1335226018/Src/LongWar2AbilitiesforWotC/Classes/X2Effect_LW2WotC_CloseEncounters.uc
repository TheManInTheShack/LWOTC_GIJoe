...
class X2Effect_LW2WotC_CloseEncounters extends X2Effect_Persistent config (LW_SoldierSkills);
...
function RegisterForEvents(XComGameState_Effect EffectGameState)
{
	...
	EventMgr.RegisterForEvent(EffectObj, 'LW2WotC_CloseEncounters', EffectGameState.TriggerAbilityFlyover, ELD_OnStateSubmitted, , UnitState);
}
...
function bool PostAbilityCostPaid(XComGameState_Effect EffectState, XComGameStateContext_Ability AbilityContext, XComGameState_Ability kAbility, XComGameState_Unit SourceUnit, XComGameState_Item AffectWeapon, XComGameState NewGameState, const array<name> PreCostActionPoints, const array<name> PreCostReservePoints)
{
	...
	SourceUnit.GetUnitValue ('LW2WotC_CloseEncountersUses', CEUsesThisTurn);
	...
	if (AbilityState != none)
	{
		if (default.CE_ABILITYNAMES.Find(kAbility.GetMyTemplateName()) != -1)
		{
			...
			if (SourceUnit.NumActionPoints() < 2 && PreCostActionPoints.Length > 0)
			{
				...
				SourceUnit.SetUnitFloatValue ('LW2WotC_CloseEncountersUses', iUsesThisTurn + 1.0, eCleanup_BeginTurn);
				//NewGameState.AddStateObject(SourceUnit);
				`XEVENTMGR.TriggerEvent('LW2WotC_CloseEncounters', AbilityState, SourceUnit, NewGameState);
			}
		}
	}
	return false;
